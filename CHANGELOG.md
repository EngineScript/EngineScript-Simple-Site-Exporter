...
Released sections describe their historical versions, including earlier tool
...
- **Security Review Follow-up**: Refresh executable metadata before identity checks, verify root-owned executables by native numeric UID, retain PHP 8.2 child exit status across process-group liveness polls, limit each process diagnostic read to 32 KiB, reject non-removable download output buffers before attempting to discard them, and stop the owned WP-CLI/database-client POSIX process group on failure or timeout.
...
- **Shell-Free WP-CLI Execution**: Database exports now run WP-CLI through a shell-free argument array with executable parent, ownership, mode, device, and inode verification; bounded diagnostics; a remaining-budget-derived timeout; live output monitoring; and bounded graceful/forced termination before process close.
- **Private-Directory-Only Actions**: Download, deletion, and cleanup paths now require the generated private directory identifier. Obsolete direct-base ZIP discovery has been removed, and downloads open and identity-check the exact file handle before clearing all output buffers and sending attachment headers.
- **Renewable Export Lease**: Replaced the transient check/set lock with an atomic, owner-bound current-site or current-network option lease. Byte-exact renewal and release prevent database collation rules or stale workers from mutating replacement owners, and owner-bound scheduled recovery removes interrupted private staging directories after expiry.
- **Export Resource Limits**: Added filterable aggregate limits for source entries, source bytes, generated bytes, elapsed time, and minimum free disk space, with live checks during database, gzip, temporary TAR, and final ZIP growth.
- **Log Data Lifecycle**: Security and error records now reject malformed stored data, redact local paths, expire after seven days through independent recurring housekeeping, and remain capped at 20 entries.
...
- **Direct Filesystem Contract**: Export operations now fail closed unless WordPress provides its direct local filesystem transport, matching the native path and archive operations used by the exporter.
- **Typed WordPress Boundaries**: Filesystem callers now consume the verified direct transport instead of rereading WordPress' mixed global, database access is narrowed once and uses the native `%i` identifier placeholder, and option, transient, filter, iterator, and request-local resource values are normalized at their boundaries.
- **Early Export Prerequisites**: The 64-bit, ZipArchive, PharData, and gzip requirements now run before directory creation, WP-CLI discovery, resource-limit evaluation, or database export.
- **PHP 8.2 Baseline**: Added native union return and parameter types, adopted PHP 8 string helpers and catch-without-variable syntax, and added an explicit 64-bit runtime requirement for archive size safety.
- **Analyzer Policy**: Removed broad Psalm file/directory suppressions, replaced them with narrow source-boundary annotations, and completed the typed-boundary cleanup so both Psalm level 1 and PHPStan max report no errors without new suppressions or weaker policy.
- **Cron Diagnostics**: Cleanup scheduling now requests `WP_Error` results and records actionable error codes and messages.
...
- **TAR Creation Contract**: Apply a private umask while PharData materializes the archive, buffer entry mutations to avoid per-file full-archive rewrites, enforce aggregate capacity against the cumulative buffered projection, and verify `0600` permissions after the first entries exist.
...
- **Admin Warning Contrast**: Updated warning text to meet WCAG AA normal-text contrast on the default WordPress admin surfaces and added forced-colors support while preserving native keyboard focus and responsive action wrapping.
